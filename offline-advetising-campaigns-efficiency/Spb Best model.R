...
library(readr)
data <- read_csv('https://app.periscopedata.com/api/wheely/chart/csv/134edbd4-efc2-0a3b-021b-19de25f26f14/189541')
...
for (metric in c('revenue', 'rides', 'riders', 'firstrides', 'secondrides', 'newusers', 
  ...
  cat(sprintf("\n\n\nMetric: %s", metric), file='report_spb.txt', append=TRUE)
  cat(sprintf("\n\nBest model: %s\n\n", best_model_type), file='report_spb.txt', append=TRUE)
  ...
  if (best_model_type == 'auto.ARIMA') {
    ...
    cat(sprintf("Average effect size: %.2f\nCI 95 [%.2f, %.2f]\nCI 80 [%.2f, %.2f]",
                ...
                file='report_spb.txt', append=TRUE)
    cat(sprintf("\n\nCumulative effect size: %.2f\nCI 95 [%.2f, %.2f]\nCI 80 [%.2f, %.2f]",
                sum(mean), sum(lower), sum(upper), sum(lower80), sum(upper80)),
                file='report_spb.txt', append=TRUE)
  } else {
    
    sink(file = 'report_spb.txt', append=TRUE)
    summary(result_best_model)
    sink()
    
  }
  
  png(paste0("spb_", metric, ".png"), 
      width=8, height=3, units="in", res=700, pointsize=10)
  par(mar=c(6, 4, 4, 2) + 0.1)
  
  if (best_model_type == 'auto.ARIMA') {
    
    fc_xts <- xts(c(fitted(best_model), result_best_model$mean), order.by=data$dd)
